...
__all__ = ['GATGRANMixtureBernoulli']
...
class GNN(nn.Module):
  ...
  def __init__(self,
               ...
               node_state_dim,
               edge_feat_dim,
               num_prop=1,  # always 1 in all configs
               num_layer=1,
               has_attention=True,
               att_hidden_dim=128,
               has_residual=False,
    ...
    self.att_use_softmax = att_use_softmax
    ...
    self.has_residual = has_residual  # always false
    self.att_hidden_dim = att_hidden_dim  # always 128
    self.has_graph_output = has_graph_output  # always false
    self.output_hidden_dim = output_hidden_dim  # always 128
    self.graph_output_dim = graph_output_dim  # always None

    self.update_func = nn.ModuleList([
        nn.GRUCell(input_size=self.msg_dim * self.num_head, hidden_size=self.node_state_dim)
      ...
      nn.Sequential(
        *[
          nn.Linear(self.node_state_dim * 2 + self.edge_feat_dim,
                    self.msg_dim),
          nn.ReLU(),
          nn.Linear(self.msg_dim, self.msg_dim * self.num_head)
        ]) for _ in range(self.num_layer)
    ])
    ...
    if self.has_attention:
      tmp_list = [
        nn.Linear(self.node_state_dim * 2 + self.edge_feat_dim,
                  self.att_hidden_dim),
        nn.ReLU(),
        nn.Linear(self.att_hidden_dim, self.num_head),
        # nn.Sigmoid()
        # TODO: why sigmoid?
      ]
      if not self.att_use_softmax:
        tmp_list.append(nn.Sigmoid())

      self.att_head = nn.ModuleList([
        nn.Sequential(*tmp_list) for _ in range(self.num_layer)
      ])
  ...
  def _prop(self, state, edge, edge_feat, layer_idx=0):
    ...
    msg = self.msg_func[layer_idx](edge_input)  # output dim: msg_dim * num_head

    ### attention on messages
    if self.has_attention:
      att_weight = self.att_head[layer_idx](edge_input)  # output dim: num_head
      if self.att_use_softmax:
        att_weight = softmax(att_weight, edge[:, 0], state.shape[0])
      msg = msg.view(-1, self.num_head, self.msg_dim) * att_weight[:, :, None]  # (-1, msg_dim, num_head)
      msg = msg.view(-1, self.num_head * self.msg_dim)
...
class GATGRANMixtureBernoulli(nn.Module):
  """ Graph Recurrent Attention Networks """

  def __init__(self, config):
    super(GATGRANMixtureBernoulli, self).__init__()
    self.config = config
    ...
    self.decoder = GNN(
        num_head=self.config.model.num_head,
        att_use_softmax=self.config.model.att_use_softmax,
        msg_dim=self.hidden_dim,
    ...
    # so theta is before sigmoid, raw logit
  ...
  def _inference(self,
    ...
    # H = self.hidden_dim  # not used
    # K = self.block_size  # not used
    ...
    node_feat = F.pad(
        node_feat, (0, 0, 1, 0), 'constant', value=0.0)  # (BCN_max + 1) X N_max/H
    ...
    if self.has_rand_feat:
      # ignore this part for now. Not used
  ...
  def forward(self, input_dict):
    ...
    if not is_sampling:  # aka, training
      B, _, N, _ = A_pad.shape
      ...
      # num_edges = log_theta.shape[0] # not used
...
def mixture_bernoulli_loss(label, log_theta, log_alpha, adj_loss_func,
  ...
  # reduce in this function: sum up logp for each order

  num_subgraph = subgraph_idx.max() + 1
  K = log_theta.shape[1]  # num of mixtures
