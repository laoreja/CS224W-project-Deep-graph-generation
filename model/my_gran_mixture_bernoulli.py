...
__all__ = ['MyGRANMixtureBernoulli']
...
class GNN(nn.Module):
  ...
  def _prop(self, state, edge, edge_feat, layer_idx=0):
    ...
    msg = self.msg_func[layer_idx](edge_input)  # output dim: msg_dim

    ### attention on messages
    if self.has_attention:
      att_weight = self.att_head[layer_idx](edge_input)  # output dim: msg_dim
      msg = msg * att_weight
...
class MyGRANMixtureBernoulli(nn.Module):
  """ Graph Recurrent Attention Networks """

  def __init__(self, config):
    super(MyGRANMixtureBernoulli, self).__init__()
    self.config = config
